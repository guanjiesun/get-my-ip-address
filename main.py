...
def get_ip_by_ipcn(client: httpx.Client) -> None:
    ...
    r = client.get(url)
    print(f'IPCN: {r.json().get("data").get("ip")}', flush=True)
...
def get_ip_by_whatismyip(client: httpx.Client) -> None:
    ...
    print(f'WhatISMyIP: {r.json().get("ip")}', flush=True)
